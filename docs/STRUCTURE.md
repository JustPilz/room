...
- Розетки, выключатели и рамки группируются по классу изделия и фактическому цвету: `fixture_class` + `fixture_color`. Розетки дополнительно группируются по наличию шторок: `fixture_shutters`.
- Для `socket_group` класс, цвет и шторки хранятся на контейнере группы, а не на отдельных механизмах. Поэтому смена параметра у одной розетки или выключателя меняет всю рамочную группу; шторки влияют только на строки розеток в комплектации.
- Панель содержит настройку `fixture_settings.default_color`: она определяет, каким цветом считать элементы с `fixture_color = default`.
- Логика: [files/js/equipment.js](../files/js/equipment.js).

### Класс, цвет и шторки электроустановочных изделий

Настенные розетки и выключатели поддерживают дополнительные свойства в `props`:

- `fixture_class` — класс изделия. Значения: `main` («Основной»), `technical` («Технический»), `premium` («Премиальный»).
...
- `fixture_settings.default_color` — проектная настройка фактического цвета для `default`. Сохраняется в экспортируемый `.plan` через `project.copy()` и читается в `project.restore()`.

По умолчанию новые элементы получают `main` и `default`, а розетки дополнительно получают `with_shutters`. В комплектации вместо «По умолчанию» выводится фактический цвет из `fixture_settings.default_color`. Селекты класса и цвета доступны в контекстных меню розеток и выключателей для обычных настенных механизмов; селект шторок доступен только у розеток. Для силовых, 380В, напольных, потолочных розеток и выводов проводов эти параметры не показываются.
