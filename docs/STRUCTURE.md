...
- Розетки, выключатели и рамки группируются по классу изделия и фактическому цвету: `fixture_class` + `fixture_color`.
- Для `socket_group` класс и цвет хранятся на контейнере группы, а не на отдельных механизмах. Поэтому смена параметра у одной розетки или выключателя меняет всю рамочную группу и её рамку.
...
### Класс и цвет электроустановочных изделий

Настенные розетки и выключатели поддерживают два дополнительных свойства в `props`:
...
По умолчанию новые элементы получают `main` и `default`. В комплектации вместо «По умолчанию» выводится фактический цвет из `fixture_settings.default_color`. Селекты доступны в контекстных меню розеток и выключателей для обычных настенных механизмов. Для силовых, 380В, напольных, потолочных розеток и выводов проводов эти параметры не показываются.
